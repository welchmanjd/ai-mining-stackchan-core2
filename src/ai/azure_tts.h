...
public:
  ...
  bool speakAsync(const String &text, const char *voice = nullptr) {
    return speakAsync(text, 0, voice);
  }
  ...
  bool consumeDone(uint32_t *outId, bool *outOk, char *outReason,
                   size_t outReasonLen);
  // legacy wrapper
  bool consumeDone(uint32_t *outId) {
    return consumeDone(outId, nullptr, nullptr, 0);
  }
...
private:
  ...
  portMUX_TYPE stateMux_; // protects state_, wav_, wavLen_
  String reqText_;
  String reqVoice_;
  String endpoint_;
