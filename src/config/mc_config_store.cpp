...
String mcConfigGetMaskedJson() {
  ...
  const bool wifi_pass_set = g_rt.wifiPass_.length() > 0;
  const bool duco_key_set  = (g_rt.ducoKey_.length() > 0) && (g_rt.ducoKey_ != "None");
  const bool az_key_set    = g_rt.azKey_.length() > 0;
  doc["wifi_ssid"] = g_rt.wifiSsid_;
  doc["wifi_pass"] = "***";
