...
public:
  ...
  void drawAll(const PanelData &p, const String &tickerText,
               bool suppressTouchBeep = false);
  // Touch snapshot: main loop should read touch (I2C) once and pass it to UI.
  // This avoids occasional I2C hangs/freezes caused by multiple touch reads per
  // frame.
  ...
  bool isSplashActive() const { return splashActive_; }
...
private:
  ...
  // from outside the draw loop. So we only *request* updates here and apply
  // them inside drawStackchanScreen().
